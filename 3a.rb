...
input.split.each.with_index do |wire, wire_index|
  ...
  wire.split(',').each do |instruction|
    ...
    distance.times do
      ...
      vs[[v[:x], v[:y]]] += 1 << wire_index
    end
  end
end

# Find the intersection the smallest distance away
p vs.select { |k, v| v > 2 }.map { |(x,y), v| [x.abs + y.abs, x, y] }.min_by { |(d, x, y)| d }.first

# END OF SOLUTION



# ------------------ VISUALIZATION PLOTTER FOLLOWS ------------------
...
vs.each do |k, v|
  0.upto(5) do |i|
    if v[i] > 0 && k[0] < 500 && k[0] >= -500 && k[1] < 500 && k[1] >= -500
