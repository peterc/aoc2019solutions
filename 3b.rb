...
# Turn directions into cartesian axis and unit distance
directions_to_magnitudes = {
  "U" => [:y, 1],
  "D" => [:y, -1],
  "L" => [:x, -1],
  "R" => [:x, 1]
...
# Store the visited points somewhere
vs = Hash.new { |h, k|
  h[k] = {
    w: 0,
    d: 0
...
# Split the input into "wires"
input.split.each.with_index do |wire, wire_index|
  # Let's set our starting co-ordinates
  v = { x: 0, y: 0, d: 0 }

  # For each instruction...
  wire.split(',').each do |instruction|
    # Figure out what axis and unit distance we need
    axis, unit = directions_to_magnitudes[instruction[0]]   

    # Then grab the actual distance (to multiply with the unit distance)
    distance = instruction[1..-1].to_i

    # For each step along the journey, track where we are
    distance.times do
      v[axis] += unit
      v[:d] += 1
      #p v[:d]

      # Don't count the origin point as an intersection
      next if v[:x] == 0 && v[:y] == 0

      # Store a record of this wire having crossed this position using a bitfield
      if vs[[v[:x], v[:y]]][:d] == 0
        vs[[v[:x], v[:y]]][:d] = v[:d]
        vs[[v[:x], v[:y]]][:w] += 1 << wire_index

      # In the case of encountering wire one, keep track of the distances
      elsif vs[[v[:x], v[:y]]][:w] == 1 && wire_index > 0
        tds << vs[[v[:x], v[:y]]][:d] + v[:d]
      end
    end
  end
end

# Pick the smallest total wire distance to an intersection
p tds.min
...
# Prepare a PNG graphic for a visualization
png = ChunkyPNG::Image.new(1000, 1000, ChunkyPNG::Color::WHITE)

# Supports plotting up to 6 wires
colors = [
  ChunkyPNG::Color.rgba(255, 0, 0, 128),
  ChunkyPNG::Color.rgba(0, 255, 0, 128),
  ChunkyPNG::Color.rgba(0, 0, 255, 128),
  ChunkyPNG::Color.rgba(0, 255, 255, 128),
  ChunkyPNG::Color.rgba(255, 0, 255, 128),
