    ...
    loop do
      ...
      # If the operand supports immediate mode..
        ...
        os.map!.with_index { |o, i|
          next o if ops[op][0][i] == 'W' && modes[i] != 2
          next o if modes[i] == 1
          if modes[i] == 2
            dos[i] = "[#{rb} + #{o}]:#{mem[o + rb]}" if DEBUG
            o += rb
            next o if ops[op][0][i] == 'W'
          else
            dos[i] = "[#{o}]:#{mem[o]}" if DEBUG
          end
      ...
      if DEBUG
        description = ops[op][1].gsub(/A|B|C/) { |l| dos[l.ord - 65] }
